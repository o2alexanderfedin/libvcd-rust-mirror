use super::*;
use crate::src::vcd::{
    vcd_get_signal_by_name, vcd_read_from_path, vcd_signal_get_value_at_timestamp,
};
...
pub(crate) extern "C" fn __main_inner(argc: i32, argv: *const *mut i8) -> Result<(), i32> {
    ...
    let vcd: *mut VcdT = vcd_read_from_path(unsafe { *argv.offset(1 as isize) });
    ...
    unsafe { printf(c"\n".as_ptr() as *mut i8 as *const i8) };
    {
        ...
        '__b6: loop {
            ...
            '__c6: loop {
                let signal_name: *const i8 = unsafe { *argv.offset(i as isize) } as *const i8;
                let signal: *mut SignalT =
                    vcd_get_signal_by_name(unsafe { &mut *vcd }, signal_name as *const i8);
                let timestamp: TimestampT = unsafe {
                    strtol(
                        unsafe { *argv.offset((i + 1) as isize) } as *const i8,
                        0 as *mut () as *mut *mut i8,
                        0,
                    )
                } as TimestampT;
                unsafe {
                    printf(
                        ...
                        timestamp,
                        vcd_signal_get_value_at_timestamp(unsafe { &mut *signal }, timestamp),
                    )
                };
                break '__c6;
            }
